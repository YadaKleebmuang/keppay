...
import { currentUser, formatTHB, obligationViewsForUser } from "@/lib/fundflow-data";
...
function Dashboard() {
  const views = obligationViewsForUser(currentUser.id);
  ...
  return (
    <AppShell variant="user">
      <PageHeader
        ...
        description="นี่คือรายการที่คุณได้รับมอบหมายให้ชำระ ยอดคงเหลือคิดจากสลิปที่ผู้ดูแลอนุมัติแล้วเท่านั้น"
      />
            ...
            <article key={v.obligation.id} className="rounded-xl border bg-card p-5 shadow-card">
              ...
              {v.collection.status === "OPEN" ? (
                <div className="mt-4 flex justify-end">
                  <Link
                    to="/pay/$obligationId"
                    params={{ obligationId: v.obligation.id }}
